...
def build_html(page, api_key) -> tuple[str, str]:
    ...
    next_plan_items_html = "".join(
        f"<tr><td colspan='2' style='padding:2px 0;color:#222;font-size:14px;line-height:1.7;font-family:Arial,sans-serif'>&bull; {html_lib.escape(i)}</td></tr>"
        for i in sections["next_plan_items"]
    )
    next_plan_section = (
        f"<tr><td colspan='2' style='padding:0 28px 20px;background:#ffffff;'>"
        f"<p style='font-size:14px;margin:0 0 10px 0;color:#666;font-weight:bold;font-family:Arial,sans-serif'>&#x1f51c; 다음 주 계획</p>"
    ...
    note_section = (
        ...
        f"<p style='font-size:14px;margin:0 0 10px 0;color:#666;font-weight:bold;font-family:Arial,sans-serif'>&#x1f4ac; 비고</p>"
        f"<p style='margin:0;color:#222;line-height:1.7;font-size:14px;font-family:Arial,sans-serif'>{html_lib.escape(sections['note_text'])}</p>"
        f"</td></tr>"
  ...
  <table width="900" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:900px;border-collapse:collapse;border:2px solid #d0d0d0;background:#ffffff;">
    ...
    <tr>
      <td colspan="2" style="padding:0 28px 20px;background:#ffffff;">
        ...
        <p style="margin:0;color:#222;line-height:1.7;font-size:14px;font-family:Arial,sans-serif;">{html_lib.escape(sections['issue_text'])}</p>
      </td>
    </tr>
    {note_section}
  </table>
